...
expandlex(void)
{
    ...
    {
        if ( !CCPPcomments ) // disable C C++ comments
        {
            struct fsm *fp;
            for (fp = fsm; fp->state>=0; fp++) 
            {
		...
		for (i=0; fp->ch[i]; i++) 
        {
			...
			switch (fp->ch[i]) 
            {
                ...
                case C_ALPH:
                    ...
                    for (j=0; j<=256; j++)
					{
						if ( DollarInMacro ) // $ in macro enabled
						{
							if ('a'<=j&&j<='z' || 'A'<=j&&j<='Z'|| j=='_' || j=='$' ) 
								bigfsm[j][fp->state] = nstate;
						}
						else
						{
							if ('a'<=j&&j<='z' || 'A'<=j&&j<='Z'|| j=='_' ) 
								bigfsm[j][fp->state] = nstate;
						}
					}	
...
gettokens(Tokenrow *trp, int reset)
{
	...
	(void)outbuf; // fix unused var outbuf
	...
	for (;;) // MAIN FOR
    {
	   continue2:
		...
		for (;;) // INNER FOR
        {
			...
			switch (state&0177) // SWITCH STATE
            {
                case S_SELF:
                ...
                default:
                    ...
                    if (c=='?') 
                    {
                        ...
                        {
                            error(FATAL, "Trigraphs not enable.");
                        }                            
                    }
