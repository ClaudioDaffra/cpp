...
#define	OUTS	16384
char	outbuf[OUTS];
char	*Outp = outbuf; // outp in windows is function
Source	*cursource;
int	nerrs;
...
void 
checkNewFlag(int argc, char **argv) // Fix Get opt
{
    for(int i=0;i<argc;i++) 
	{
		if(argv[i][0]=='-'&&argv[i][1]=='T') Trigraphs=0;
		if(argv[i][0]=='-'&&argv[i][1]=='C') CCPPcomments=0;
		if(argv[i][0]=='-'&&argv[i][1]=='$') DollarInMacro=0;    		
	}
}
